...
func TestNewRoute(t *testing.T) {
	...
	body, status := getNewResponse(t, router)
...
func TestInvalidGuesses(t *testing.T) {
	...
	for _, guess := range guesses {
		...
		t.Run(guess.name, func(t *testing.T) {
			...
			body, status := getGuessResponse(t, guess.guess, router)
...
func TestGuessFlow(t *testing.T) {
	body, _ := getNewResponse(t, router)
	...
	for _, guess := range guesses {
		t.Run(fmt.Sprintf("guess '%s' yields '%s'", guess.guess.Guess, guess.want), func(t *testing.T) {
			...
			body, _ = getGuessResponse(t, guess.guess, router)
...
func TestExhaustedGuesses(t *testing.T) {
	...
	for i := 1; i <= guessLimit+1; i++ {
		if i <= guessLimit {
			body, _ = getGuessResponse(t, guess, router)
			json.Unmarshal(body, &response)
			assert.Equal(t, guessLimit-i, response.GuessesRemaining)
		} else {
			body, status = getGuessResponse(t, guess, router)
			json.Unmarshal(body, &gameError)
...
func getNewResponse(t *testing.T, router *gin.Engine) ([]byte, int) {
...
func getGuessResponse(t *testing.T, guess GuessRequest, router *gin.Engine) ([]byte, int) {
	recorder := httptest.NewRecorder()
